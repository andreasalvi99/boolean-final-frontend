...
export default function OrderSuccess() {
  ...
  function fetchOrderDetails() {
    axios
      .get(`https://laravel-final-backend.onrender.com/api/orders/${orderId}`)
      .then((response) => {
  ...
  const cartAmount =
    order?.comics?.reduce((total, comic) => {
      return total + comic.pivot.quantity * Number(comic.pivot?.price);
    }, 0) ?? 0;
  ...
  return (
    <>
      ...
      <p className="oswald-special fs-5">
        Il tuo ordine è stato inviato, a breve riceverai una mail di conferma
        con il link per il tracciamento. Grazie per aver scelto Nextpanel.
      </p>
